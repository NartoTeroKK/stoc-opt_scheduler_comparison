...
from stoc_opt_scheduler_comparison.data_load.transforms import (
    standard_normalize,
    mnist_normalize,
)
...
def load_synthetic(
    test_size: float, seed: int, n_samples: int = 50000, class_sep: float = 0.8
) -> Dataset:
    """
...
def load_mnist(test_size: float, seed: int) -> Dataset:
    ...
    y = np.concatenate(
        [full_train.targets.numpy(), official_test.targets.numpy()], axis=0
    )
...
def get_dataloaders(
    ...
    **kwargs,
    ...
    return {
        "train": DataLoader(
            TensorDataset(
                torch.from_numpy(ds.X_train), torch.from_numpy(ds.y_train).long()
            ),
        ...
        "test": DataLoader(
            TensorDataset(
                torch.from_numpy(ds.X_test), torch.from_numpy(ds.y_test).long()
            ),
            batch_size=batch_size,
            shuffle=False,
        ),
